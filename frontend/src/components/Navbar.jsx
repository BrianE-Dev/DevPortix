...
const ACCENT_NAV_STYLES = {
  blue: {
    navDark:
      "bg-gradient-to-r from-slate-950/95 via-slate-900/95 to-blue-950/95 border-white/10",
    navLight:
...
const Navbar = () => {
  ...
  const [accent, setAccent] = useState(() =>
    LocalStorageService.getDashboardAccent(),
  );
  const location = useLocation();
  const navigate = useNavigate();
  const { user, logout, isAuthenticated, getDashboardPath, updateProfile } =
    useAuth();
  ...
  const getProfileMenuKey = () => {
    ...
    if (role === "student")
      return { storageKey: "student", menuKey: "profile" };
    if (role === "instructor")
      return { storageKey: "instructor", menuKey: "profile" };
    if (role === "professional")
      return { storageKey: "professional", menuKey: "profile" };
    if (role === "organization")
      return { storageKey: "organization", menuKey: "profile" };
    return null;
  };
  ...
  return (
    <nav
      className={`navbar-theme-preserve sticky top-0 w-full backdrop-blur-lg z-50 border-b ${
        isDark ? accentStyles.navDark : accentStyles.navLight
      }`}
    >
          ...
          <div className="flex items-center">
            ...
            >
              <div className="flex flex-col justify-between items-center group cursor-pointer">
                <BrandLogo
                  className="h-10 w-auto max-w-[10rem] lg:h-12 lg:max-w-[13rem]"
                  alt="DevPortix logo"
                />
                {!isPortfolioPage && (
                  <span
                    className={`mt-1 text-xs sm:text-sm group-hover:text-violet-300 ${isDark ? "text-white" : "text-white/95"}`}
                  >
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            >
              {isDark ? (
                ...
                <Moon
                  className="w-5 h-5 text-white [filter:drop-shadow(0_0_1px_rgba(255,255,255,0.95))]"
                  strokeWidth={2.5}
                />
              )}
            </button>
            ...
            {isAuthenticated ? (
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span
                    className={`text-sm font-medium ${isDark ? "text-gray-100" : "text-white"}`}
                  >
                    {user?.username}
                  </span>
                </div>
      ...
      {isOpen && (
        <div
          className={`md:hidden border-b ${isDark ? "bg-gray-900 border-gray-800" : "bg-slate-950/95 border-slate-700/60"}`}
        >
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {!isPortfolioPage && (
              <div
                className={`pt-4 border-t ${isDark ? "border-gray-800" : "border-slate-700/60"}`}
              >
                {isAuthenticated ? (
                  <div className="space-y-2">
                    <div className="flex items-center px-3 py-2">
                      ...
                      >
                        <User
                          className={`w-4 h-4 ${accentStyles.avatarIcon}`}
                        />
                      </button>
                      <div>
                        <p
                          className={`text-sm font-medium ${isDark ? "text-white" : "text-white"}`}
                        >
                          {user?.username}
                        </p>
                        <p
                          className={`text-xs ${isDark ? "text-gray-300" : "text-slate-200"}`}
                        >
                          {user?.email}
                        </p>
                      </div>
